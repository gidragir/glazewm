...
pub fn resize_tiling_container(
  ...
  // If the container is a top-level column on a horizontal workspace
  // (infinite canvas), resize it independently without constraining
  // against or resizing siblings.
  if let Some(parent) = container_to_resize.parent()
    && parent.as_workspace().is_some()
  {
